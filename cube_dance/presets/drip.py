"""`drip` - light drips down the posts under gravity and pools at the base."""
...
TRIGGERS = [
    ...
    Trigger("flash", (180, 255, 255), lambda m, s, c: el.ColorStab(m, c, gain=s, release=0.28)),
    Trigger("spark", (200, 240, 255), lambda m, s, c: el.SparkBurst(m, c, count=30, release=0.6)),
    Trigger("wipe", (120, 255, 255), lambda m, s, c: el.Wipe(m, c, axis=1, dur=0.9, gain=1.0*s)),
]


def build(engine) -> None:
    engine.add(fx.GravityDrip(engine.model))
