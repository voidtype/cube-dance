"""`panels` - Daft-Punk-pyramid style chevrons climbing the faces."""
...
TRIGGERS = [
    ...
    Trigger("wipe", (120, 200, 255), lambda m, s, c: el.Wipe(m, c, axis=1, dur=0.9, gain=1.0*s)),
    Trigger("shock", (255, 120, 200), lambda m, s, c: el.Shockwave(m, c, dur=0.6, gain=1.4*s)),
    Trigger("glint", (200, 255, 220), lambda m, s, c: el.SparkBurst(m, c, count=30, release=0.6)),
]


def build(engine) -> None:
    engine.add(fx.PanelSequencer(engine.model))
